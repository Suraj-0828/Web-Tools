...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $transport = (new Swift_SmtpTransport('mail.ramkrishnatechnology.com', 587))
        ->setUsername('no-reply@ramkrishnatechnology.com')
        ->setPassword('mSV*hI~M]aJ@');

    // Create the Mailer using the created Transport
    $mailer = new Swift_Mailer($transport);

    // Create a message  ->  info@ramkrishnatechnology.com
    $message = (new Swift_Message('Contact Form Query'))
        ->setFrom(['no-reply@ramkrishnatechnology.com' => 'Contact Form Query'])
